...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($titulo) || empty($ambiente) || empty($resumen) || empty($topico) || empty($criterios)) {
        ...
        $resultado = $db->createFuncionalidad($titulo, $ambiente, $resumen, $criterios, $user_id, $topico);
        
        if ($resultado) {
            $mensaje = 'Solicitud de funcionalidad creada correctamente.';
            $tipo = 'success';
        } else {
            $mensaje = 'Ha ocurrido un error al crear la solicitud. Por favor, inténtelo de nuevo.';
            $tipo = 'danger';
        }
    }
