...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validaciones
    if (empty($id_error) || empty($titulo) || empty($descripcion) || empty($topico)) {
        ...
        if ($_SESSION['rol'] === 'usuario') {
            ...
            if (!$es_propietario) {
                $mensaje = 'No tiene permiso para modificar este reporte de error.';
                $tipo = 'danger';
        ...
        // Intentar actualizar el reporte de error
        $resultado = $db->updateError($id_error, $titulo, $descripcion, $topico);
        
        if ($resultado) {
            $mensaje = 'Reporte de error actualizado correctamente.';
            $tipo = 'success';
        } else {
            $mensaje = 'Ha ocurrido un error al actualizar el reporte o el error está en progreso.';
            $tipo = 'danger';
        }
    }
